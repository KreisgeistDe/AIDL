...
The bridge semantic envelope remains `aidl.m10.1-normalized/v2` for the base bridge and `aidl.m10.1-production/v5` for production normalization. Policy, execution and declaration-family completion add only deterministic audit envelopes `aidl.m10.1-operation-policy-parity/v1`, `aidl.m10.1-operation-execution-parity/v1` and `aidl.m10.1-declaration-family-parity/v1`; they do not alter canonical hashes, parser behavior, CLI JSON, runtime behavior or Canonical IR schemas.

M10.1 remains open at **M10.1-09**. M10.1-08 is complete because all 48 frozen canonical semantic declaration kinds now have a deterministic contract-derived production disposition: ten are backed by existing Production Normalization admission and 38 remain explicit intentional fail-closed non-admissions. M10.5-03 remains blocked for semantic front-end/AST/IR work until all relevant M10.1 packages through closure certification are complete.
