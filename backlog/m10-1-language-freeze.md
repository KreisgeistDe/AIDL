...
The bridge semantic envelope remains `aidl.m10.1-normalized/v2` for the base bridge and `aidl.m10.1-production/v5` for production normalization. Policy and execution completion add only deterministic audit envelopes `aidl.m10.1-operation-policy-parity/v1` and `aidl.m10.1-operation-execution-parity/v1`; they do not alter canonical hashes, parser behavior, CLI JSON, runtime behavior or Canonical IR schemas.

M10.1 remains open at **M10.1-08**. M10.1-07 is complete because every remaining operation-execution concept now has an executable frozen-contract disposition: range/default are Production-Parity, while generic TypeRef arguments, non-range constraints, operation generics, `idempotency` and `transaction` are explicit fail-closed exclusions. M10.5-03 remains blocked for semantic front-end/AST/IR work until all relevant M10.1 packages through closure certification are complete.
