...
A critical post-P3 self-description correction is now a blocking Core sub-gate before P4 or any semantics-dependent M10.5 continuation. The earlier `produces(type)` carrier-capability interpretation, host `type-position` marker, separate semantic `type` hierarchy, and `Args=void` model are superseded. `declaration` is the central self-referential meta-class and itself a Type; direct Core authors `type` only as an alias/syntax view of that same category. Every visible named declaration, including declaration-kind names, concrete entities/enums and named queries, is TypeRef-addressable by generic symbol identity. `args(...)` denotes an open parameter set, while no args specification and explicit `()` are the same closed zero-parameter model. Results use only the existing `->` envelope position; specialized result contracts require their result TypeRef. Generated meta-IR remains deterministic non-authoritative output-only. Until this amended correction is independently architecture-reviewed, exact-head validated and integrated, PR #99, M10.5-03+ and all P4/downstream semantic feature work remain frozen. `spec/core-authority-transition-v1.json` is the durable correction-state contract.
