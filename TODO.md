...
- [ ] **G1 — Core authority integration/flip.** **Current implementation candidate.** This branch makes normative Core/Core-authored modules the sole permanent semantic authority and requires production use of revision-4 compatibility data to pass an exact Core-authored compatibility-projection binding. G1 remains unchecked until this candidate is independently validated and integrated.

On the G1 candidate, `spec/language-surface-v1.json` revision 4 is no longer an independent production semantic authority. It remains frozen compatibility/migration evidence and may be consumed by legacy production normalization only after `spec/core.authority.aidl` and `spec/core.compatibility.aidl` validate and authorize its exact Git-blob identity. PR #88's Kotlin type-construction implementation remains bounded evidence; semantics-dependent Kotlin adaptation remains a separate post-G1 phase.
...
The former execution order `M10.1 -> M10.2 -> M10.3 -> M10.5` is interrupted by the blocking Core gate. Semantics-dependent work must follow `D0 -> I1 -> I2 -> V1 -> G1` before resuming M10.5 adaptation. Existing language-neutral parity harness/infrastructure may remain as evidence, but it cannot define new parser/AST/type semantics independently of Core.

## M10.5 Kotlin compiler-core migration gate

[M10.5 — Kotlin Compiler-Core Migration Gate](backlog/m10-5-kotlin-compiler-migration.md) remains blocked for new semantics-dependent work until G1 is independently validated and integrated. Python and revision-4 artifacts remain compatibility/conformance evidence; they are not permission to widen Kotlin semantics as part of G1.

- [ ] **M10.5-01 — Refresh the Python parity baseline and differential harness.** PR #77 is the focused candidate as retained revision-4 compatibility evidence. Completion requires fresh independent validation and integration; any future baseline refresh must be reconciled with Core-owned authority after G1.
- [ ] **M10.5-02 and later Kotlin work.** Blocked until M10.5-01 is durably integrated and the Core authority flip is independently validated/integrated. PR #88's integrated TypeConstruction slice is retained as bounded evidence; generic Core TypeRef adaptation is explicitly deferred until after G1.
