...
The former execution order `M10.1 -> M10.2 -> M10.3 -> M10.5` was interrupted by the Core gate. D0, I1, I2, V1 and G1 are complete, and the refreshed M10.5-01 parity-baseline gate has now also been independently validated and integrated. Existing language-neutral parity harness/infrastructure remains evidence only; future semantic work remains governed by Core.

## M10.5 Kotlin compiler-core migration gate

[M10.5 — Kotlin Compiler-Core Migration Gate](backlog/m10-5-kotlin-compiler-migration.md) is unblocked by the completed Core gate, but its internal sequencing remains mandatory. Python and revision-4 artifacts remain compatibility/conformance evidence under Core-owned authority.

- [x] **M10.5-01 — Refresh the Python parity baseline and differential harness.** Independently validated at exact PR #94 head `9228f94302c1fbbc6cd5fc0b5fc7af9231071d76` and integrated on main as `fcfc3fc92e6577270dbf89be22c4ddfac5c187a9`. The accepted post-G1 baseline was derived from exact `main@f471fd9c1ee808ff9557d4a9f6bdf8b092d9c2c5`, preserves Core/Core-authored modules as the sole permanent semantic authority, separately fingerprint-binds the three Core authority inputs, preserves the inherited six parity bindings and exact `source`/`config`/`profile` runner identity, and keeps revision 4 compatibility/conformance-only. PR #77 remains already-merged historical/provisional evidence on main via `1574963eed95a2f80c1cdc47f48a3eaa39df4a4b`; it is not a future integration target. The focused durable-state reconciliation was independently validated at exact PR #95 head `2a26b1deff1baa5504e5e714222e8726181c7d5a` and integrated on main as `3fa5c969338d1f0dc6b9bc573e70c7004f53aceb`.
- [x] **M10.5-02 — Kotlin Multiplatform compiler skeleton / Gate 02.** Independently validated at exact PR #97 head `dfb3af756f2c726f925ee16bddecebe004e5c781` and squash-integrated on main as `5ca01218246982e4b21701254bf3f1844b103014` with sole parent `11fd92d991cf2ff9f5354589dbfd0c0397908621`. One platform-neutral `CommonCompilerBoundary` is consumed by both JVM and Linux/Native adapters; host integration remains adapter-local, Python remains the reference/conformance implementation, Core-owned authority and the accepted parity contract remain unchanged, Kover's configured >=95% line and branch gates passed, and the Native executable linked successfully. Resulting-main push Validation #409 run `34760402346` and Kotlin Compiler Skeleton #66 run `34760402339` completed successfully.
- [ ] **M10.5-03 and later Kotlin semantic work.** Gate 03 remains incomplete. Existing `SourceProjection`, `NameResolution`, and `TypeConstruction` code is bounded historical evidence only and does not establish Gate-03 completion or move semantic authority away from Python/Core.
