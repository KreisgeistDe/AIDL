...
The post-P3 binding-E​BNF correction is complete. The earlier `produces(type)` carrier-capability interpretation, host `type-position` marker, separate semantic `type` hierarchy, and `Args=void` model are superseded. `declaration` is the central self-referential meta-class and itself a Type; direct Core authors `type` only as an alias/syntax view of that same category. Every visible named declaration, including declaration-kind names, concrete entities/enums and named queries, is TypeRef-addressable by generic symbol identity. The universal declaration envelope is `[export] type-reference identifier [generic-parameters] [named-arguments] [-> type-reference] [declaration-body] [statement-end]`: omitted args are the closed zero-parameter contract, explicit empty `()` is invalid, fixed args are named TypeRef parameters, and open args use a free binder such as `(args?: ...)`. Nullable/generic TypeRefs, expression-valued body entries, grammar-incomplete newline continuation, nested comments, ranges/lists/postfix expressions, and normal/raw multiline string interpolation are part of the binding boundary. Generated meta-IR remains deterministic non-authoritative output-only. PR #104 exact head `9efde375636bb7deed1c901842087169cb5b2763` independently passed architecture/exact-head validation and was serially squash-integrated on main as `363f2a7a20f75d9fa7f46aea19e015bcdf121c33`, with successful post-merge Validation and Kotlin CI. The binding correction no longer freezes M10.5; subsequent work remains governed by its own roadmap gates. `spec/core-authority-transition-v1.json` records the completed correction state.

After G1, `spec/language-surface-v1.json` revision 4 is no longer an independent production semantic authority. It remains frozen compatibility/migration evidence and may be consumed by legacy production normalization only after `spec/core.authority.aidl` and `spec/core.compatibility.aidl` validate and authorize its exact Git-blob identity. PR #88's Kotlin type-construction implementation remains bounded evidence; semantics-dependent Kotlin adaptation proceeds only through the separately gated M10.5 sequence under direct Core authority.
...
The former execution order `M10.1 -> M10.2 -> M10.3 -> M10.5` was interrupted by the Core gate. D0, I1, I2, V1 and G1 are complete, the binding Core EBNF correction was independently reviewed and integrated as PR #104, and the refreshed M10.5-01 parity-baseline gate was independently validated and integrated. Existing language-neutral parity harness/infrastructure remains evidence only; current semantic work is governed by direct Core authority and the M10.5 gate sequence.

## M10.5 Kotlin compiler-core migration gate

[M10.5 — Kotlin Compiler-Core Migration Gate](backlog/m10-5-kotlin-compiler-migration.md) has completed Gates 01 through 03 and is no longer blocked by the binding Core EBNF correction. Python and revision-4 artifacts remain compatibility/conformance evidence under direct Core-owned authority. Gate 03 was independently certified complete on exact main `7bd092ed81906dd471c198792eadd96bcd1853d7` after integration of PR #113. M10.5-04 is now eligible only through bounded Canonical-IR and semantic-query parity packages; Gate 04 remains incomplete.
...
- [ ] **M10.5-03 and later Kotlin semantic work.** Gate 03 itself is complete: the independent closure audit on exact main `7bd092ed81906dd471c198792eadd96bcd1853d7` accepted the integrated parsing/source-projection, resolution, TypeConstruction/type-checking, serialization, materialization, location, recursion and diagnostic-parity matrix. The combined item remains open because M10.5-04+ is not complete. The first authorized Gate-04 package is deliberately bounded to Canonical-IR parity for the already-covered domain-declaration surface; it does not close Gate 04 or authorize M10.5-05+.
