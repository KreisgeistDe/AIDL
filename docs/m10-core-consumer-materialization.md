...
The normative Consumer body contains exactly these clause families: `service`, `idempotency`, `retry`, `start`, `call`, and `transaction`. Every accepted Consumer fact now has one of two outcomes: lossless projection or stable rejection before IR.
...
- Effectful Consumer idempotency cardinality remains owned by `AIDL-DIST411`: exactly one idempotency clause is required for effectful Consumers, while pure Consumers may omit it. The currently closed inline form `idempotency: EXPRESSION retain DURATION` materializes `key`, the canonical Consumer scope, and `retentionMs`. The normative multiline block form is parsed but its key/scope/retain properties are not currently associated losslessly by the parser/IR projection, so it is rejected before IR with `AIDL-T005` rather than silently dropped. Malformed inline retention is likewise rejected with `AIDL-T005`.
...
No new diagnostic code is introduced. Existing `AIDL-DIST415` and `AIDL-DIST411` ownership remains authoritative for their respective policy failures; the focused closure regressions verify those failures do not receive a Consumer `AIDL-T005` duplicate. Consumer-specific materialization-only losses are collected through `tools/compiler_consumer_materialization.py` and use the existing `AIDL-T005` ownership.

## Executable evidence

`tools/test_core_consumer_materialization_closure.py` proves the whole Source→Validate→IR boundary directly:

- deterministic full-document builds from the same source path;
...
- inline idempotency key, canonical scope, and retention;
...
- stable `AIDL-T005` rejection for multiline/malformed idempotency, service mismatch, immediate/malformed retry, mutation/saga/unresolved/unprojectable starts, `call`, and Consumer `transaction`.
