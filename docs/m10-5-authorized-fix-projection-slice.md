...
The Kotlin projection does **not** derive diagnostic meaning and never applies edits. Its input is the caller-owned diagnostic stream for the same exact saved or unsaved source snapshot, so existing compiler diagnostics remain the semantic authority. Source identity remains snapshot-local: diagnostics from a different source/root id are ignored. A JVM differential signature is pinned by a Python regression that invokes the real `create_compiler_snapshot(...)` and `authorized_snapshot_fixes(...)` helpers.
