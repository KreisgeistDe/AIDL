...
- Validate: `tools/test_core_domain_type_ir_semantics.py`, `tools/test_m5_fixture_corpus.py`, `tools/test_core_enum_ir_semantics.py`, `tools/test_core_error_contract_ir_semantics.py`, `tools/test_core_event_ir_semantics.py`, `tools/test_core_topic_materialization_contract.py`, `tools/test_core_typecheck.py`, and `tools/test_core_value_ir_semantics.py`
...
## Error Validate/IR closure

The closed Core Error IR contains exactly four source-derived contract facts: `code`, `safeMessage`, `retry`, and `transportStatus`. Before this closure, `_declaration(error)` could default a missing code to the declaration name, a missing message to the empty string, a missing retry to `never`, and a missing HTTP status to `500`; it could also drop `localizationKey` and valid-looking Error body payload facts. The wider source retry grammar admits `immediate`, `backoff`, and `after duration`, while the closed IR schema uses a different retry vocabulary, so silently copying or normalizing those forms would not be a truthful 1:1 projection.

`tools/compiler_error_materialization.py` now places an Error-specific pre-IR boundary in front of that projector. Existing exported-contract failures already owned by `AIDL-T003` remain authoritative: missing/duplicate public contract clauses, empty code/message, invalid public HTTP status, and invalid retry classes do not receive duplicate materialization diagnostics. Stable source-located `AIDL-T005` is used only for materialization-only loss: `localizationKey`, otherwise valid-looking non-contract body facts, non-exported missing or duplicate contract clauses that would otherwise receive IR defaults, and every valid retry source form other than exact `retry never` because no other source spelling has an already normative 1:1 mapping to the closed IR retry value.

`tools/test_core_error_contract_ir_semantics.py` proves the accepted Source→Validate→IR path with one explicit exported Error. Repeated analysis of the same source path yields identical full Canonical IR; the test proves exact canonical FQN/declaration identity, explicit non-empty code, explicit safeMessage, unchanged schema-valid `never` retry, explicit 4xx/5xx transport status, the matching source-map entry, and full `spec/ir.schema.json` validity. Its negative matrix proves `AIDL-T005` rejection for localization, payload/body loss, unsupported retry source forms, and non-exported default/duplicate cases, while a separate ownership matrix proves established exported failures still produce `AIDL-T003` without an Error `AIDL-T005` duplicate.
...
`decl.error/generate` and `decl.error/ide` remain `partial`. No grammar, IR schema/version, localization field, retry normalization, Consumer/App/Module/Import closure, runtime/generator, M16.5, release/protection workflow, or IDE behavior is widened by this closure.
