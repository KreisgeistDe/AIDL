...
def load_config(root: Path = ROOT, path: Path | None = None) -> ReleaseConfig:
    ...
    if raw["publication"] != "disabled":
        raise ReleaseError("M9-05 release workflow must keep publication disabled")
    return ReleaseConfig(
        schema_version=raw["schemaVersion"],
        distribution=raw["distribution"],
        version=raw["version"],
        tag=raw["tag"],
        notes_source=raw["notesSource"],
        contract_glob=raw["contractGlob"],
...
def tracked_contracts(root: Path, config: ReleaseConfig) -> list[Path]:
    ...
    paths = [Path(name) for name in names if name.endswith(".json")]
...
def validate_contract(root: Path, config: ReleaseConfig, *, tag: str | None = None) -> list[Path]:
    ...
    if distribution != config.distribution:
        raise ReleaseError(
            f"release distribution {config.distribution!r} does not match pyproject {distribution!r}"
        )
    if version != config.version:
        raise ReleaseError(
            f"release version {config.version!r} does not match pyproject version {version!r}"
        )
    ...
    expected_heading = f"## {config.version} — unreleased toolchain baseline"
    if expected_heading not in notes:
        raise ReleaseError(
            f"release notes source must contain exact version heading {expected_heading!r}"
        )
...
def expected_wheel_name(config: ReleaseConfig) -> str:
    normalized = config.distribution.replace("-", "_")
    return f"{normalized}-{config.version}-py3-none-any.whl"
...
def _build_wheel(root: Path, destination: Path, config: ReleaseConfig, epoch: int) -> Path:
    ...
    if len(wheels) != 1:
        raise ReleaseError(f"expected exactly one wheel, found {[path.name for path in wheels]}")
...
def _checksum_lines(paths: Iterable[Path], base: Path) -> str:
    entries = sorted(paths, key=lambda path: path.relative_to(base).as_posix())
    return "".join(f"{_sha256(path)}  {path.relative_to(base).as_posix()}\n" for path in entries)
...
def build_bundle(root: Path, output: Path, config: ReleaseConfig, *, tag: str | None = None) -> Path:
    ...
    component_paths: list[Path] = [wheel_target]
    for source in contracts:
        target = contracts_dir / source.name
        shutil.copyfile(root / source, target)
        component_paths.append(target)
    ...
    shutil.copyfile(root / config.notes_source, notes_target)
    component_paths.append(notes_target)
    ...
    manifest = {
        ...
        "wheel": {
            "path": wheel_target.relative_to(stage).as_posix(),
            "sha256": _sha256(wheel_target),
        },
        "releaseNotes": {
            "path": notes_target.relative_to(stage).as_posix(),
            "sha256": _sha256(notes_target),
            "sourcePath": config.notes_source,
        },
        "contracts": released_contracts,
    }
    _write_json(manifest_target, manifest)
    component_paths.append(manifest_target)

    checksums = stage / "SHA256SUMS"
    checksums.write_text(_checksum_lines(component_paths, stage), encoding="utf-8")

    archive = output / f"{stage.name}.tar.gz"
    _archive_tree(stage, archive, epoch)
    archive_checksum = output / f"{archive.name}.sha256"
    archive_checksum.write_text(f"{_sha256(archive)}  {archive.name}\n", encoding="utf-8")
...
def _expected_stage_files(root: Path, config: ReleaseConfig) -> set[str]:
    ...
    return {
        expected_wheel_name(config),
        f"{config.distribution}-{config.version}-release-notes.md",
        f"{config.distribution}-{config.version}-release-manifest.json",
        "SHA256SUMS",
...
def validate_bundle(root: Path, output: Path, config: ReleaseConfig, *, tag: str | None = None) -> None:
    ...
    actual_root = {path.name for path in output.iterdir()}
    if actual_root != expected_root:
        raise ReleaseError(
            f"release output entries differ: expected {sorted(expected_root)}, got {sorted(actual_root)}"
        )
    expected_stage = _expected_stage_files(root, config)
    actual_stage = _all_relative_files(stage)
    if actual_stage != expected_stage:
        raise ReleaseError(
            f"release stage files differ: expected {sorted(expected_stage)}, got {sorted(actual_stage)}"
        )
    ...
    if manifest.get("publication") != "disabled":
        raise ReleaseError("release manifest must keep publication disabled")

    contract_entries = manifest.get("contracts")
    expected_contract_sources = [path.as_posix() for path in contracts]
    if not isinstance(contract_entries, list):
        raise ReleaseError("release manifest contracts must be a list")
    actual_contract_sources = [entry.get("sourcePath") for entry in contract_entries]
    if actual_contract_sources != expected_contract_sources:
        raise ReleaseError(
            f"release manifest contract inventory differs: expected {expected_contract_sources}, got {actual_contract_sources}"
        )
    for entry in contract_entries:
        release_path = entry["releasePath"]
        target = stage / release_path
        if entry["sha256"] != _sha256(target):
            raise ReleaseError(f"release manifest checksum mismatch for {release_path}")

    wheel_entry = manifest.get("wheel", {})
    notes_entry = manifest.get("releaseNotes", {})
    for entry_name, entry in (("wheel", wheel_entry), ("releaseNotes", notes_entry)):
        path_value = entry.get("path")
        if not isinstance(path_value, str):
            raise ReleaseError(f"release manifest {entry_name} path is missing")
        if entry.get("sha256") != _sha256(stage / path_value):
            raise ReleaseError(f"release manifest checksum mismatch for {entry_name}")

    checksum_targets = [
        stage / relative
        for relative in sorted(expected_stage)
        if relative != "SHA256SUMS"
    ]
    expected_checksums = _checksum_lines(checksum_targets, stage)
    if (stage / "SHA256SUMS").read_text(encoding="utf-8") != expected_checksums:
        raise ReleaseError("SHA256SUMS does not exactly match release components")

    expected_archive_line = f"{_sha256(archive)}  {archive.name}\n"
    if archive_checksum.read_text(encoding="utf-8") != expected_archive_line:
    ...
    with tarfile.open(archive, "r:gz") as tar:
        ...
        for member in members:
            ...
            stage_relative = member.removeprefix(f"{stage_name}/")
            if extracted.read() != (stage / stage_relative).read_bytes():
                raise ReleaseError(f"archive content mismatch for {member}")


def compare_output_trees(first: Path, second: Path) -> None:
    first_files = _all_relative_files(first)
    second_files = _all_relative_files(second)
    if first_files != second_files:
        raise ReleaseError(
            f"reproducibility file set differs: {sorted(first_files)} != {sorted(second_files)}"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "verify-contract":
            ...
            print(
                f"release contract: {config.distribution} {config.version}, "
                f"{len(contracts)} JSON contracts, publication disabled"
            )
