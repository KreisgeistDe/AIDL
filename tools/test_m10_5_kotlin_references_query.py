...
class KotlinReferencesQueryParityTest(unittest.TestCase):
    ...
    def _render(self, label: str, result) -> str:
        ...
        usages = ",".join(
            f"{usage.source_path.stem}:{usage.location.line}:{usage.location.column}:{usage.location.offset}:{usage.length}"
    ...
    def test_python_authority_matches_pinned_references_signature(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            invalid_snapshot = create_compiler_snapshot([root], {consumer_path: consumer + "§"})
            ...
            actual = "\n".join((
                ...
                self._render("memory-lexical-failure", find_snapshot_usages(invalid_snapshot, consumer_path, 0)),
            ))
            expected = (PARITY / "references-query.signature").read_text(encoding="utf-8").rstrip()
    ...
    def test_isolated_roots_do_not_leak(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for name, module in (("root-a", "a"), ("root-b", "b")):
                ...
                self.assertEqual(f"{module}.A", result.target.fully_qualified_name)
