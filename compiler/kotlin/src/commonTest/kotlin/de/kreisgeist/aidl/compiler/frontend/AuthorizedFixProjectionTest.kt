...
"""
    private fun diagnostic(
        ...
        sourceId: String = "authorized-fixes.aidl",
    ) = ProjectedAuthorizedFixDiagnostic(code, sourceId, text.indexOf("consumer"), fixes)
    ...
    fun staleOffsetsAndUnrelatedDiagnosticsFailClosed() {
        ...
        val unrelated = ProjectedAuthorizedFixDiagnostic("AIDL-DIST411", "other-root/authorized-fixes.aidl", 0, fix)
    ...
    @Test
    fun missingOrMalformedSnapshotFailsClosed() {
        val query = ProjectAuthorizedFixQuery.fromSnapshot(
            listOf("authorized-fixes.aidl" to source),
            listOf(diagnostic()),
        )
